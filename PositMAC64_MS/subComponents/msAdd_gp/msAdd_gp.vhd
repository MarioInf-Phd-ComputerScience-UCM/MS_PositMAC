...
architecture estr of msAdd_gp is
...
begin
  ...
  --load <= '1';--We load when indicated and there is a misprediction
  ...
  for i in 0 to (N/K-1) generate
    ...
    if_gp_KS_x_i: if (ADDER_ALGORITHM = KS or ADDER_ALGORITHM = UNKNOWN) generate      --KOGGE-STONE SELECTION
      gp_KS_x_i: gp_kogge_stone generic map(K,LOG_K)
                  port map(a_mux((K*(i+1)-1) downto K*i),b_mux((K*(i+1)-1) downto K*i), cins_mux(i),
                  Gz_aux(i),Pz_aux(i),z_aux((K*(i+1)-1) downto K*i),couts_z_aux(i));
    end generate;

    if_gp_BK_x_i: if (ADDER_ALGORITHM = BK) generate      --BRENT-KUNG SELECTION
      gp_BK_x_i: gp_brent_kung generic map(K,LOG_K)
                  port map(a_mux((K*(i+1)-1) downto K*i),b_mux((K*(i+1)-1) downto K*i), cins_mux(i),
                  Gz_aux(i),Pz_aux(i),z_aux((K*(i+1)-1) downto K*i),couts_z_aux(i));
    end generate;

    if_gp_RCA_x_i: if (ADDER_ALGORITHM = RCA) generate    --RCA SELECTION
      gp_RCA_x_i: gp_rca generic map(K,LOG_K)
                  port map(a_mux((K*(i+1)-1) downto K*i),b_mux((K*(i+1)-1) downto K*i), cins_mux(i),
                  Gz_aux(i),Pz_aux(i),z_aux((K*(i+1)-1) downto K*i),couts_z_aux(i));
    end generate;

    regG_i: dff port map(rst,clk,'1',Gz_aux(i),rGz_aux(i));
    regP_i: dff port map(rst,clk,'1',Pz_aux(i),rPz_aux(i));
  ...
  if (N mod K /= 0) generate
    ...
    if_gp_KS_x_Last: if (ADDER_ALGORITHM = KS or ADDER_ALGORITHM = UNKNOWN) generate      --KOGGE-STONE SELECTION
      gp_KS_x_Last: gp_kogge_stone generic map(K,LOG_K)
                      port map(A_last,B_last,cins_mux(N/K),
                      G_last,P_last,z_last,couts_z_aux(N/K));
    end generate;

    if_gp_BK_x_Last: if (ADDER_ALGORITHM = BK) generate      --BRENT-KUNG SELECTION
      gp_BK_x_Last: gp_brent_kung generic map(K,LOG_K)
                      port map(A_last,B_last,cins_mux(N/K),
                      G_last,P_last,z_last,couts_z_aux(N/K));
    end generate;  
    
    if_gp_RCA_x_Last: if (ADDER_ALGORITHM = RCA) generate    --RCA SELECTION
    ...
    Gz_aux(N/K) <= G_last;
    Pz_aux(N/K) <= P_last;
    regG_last: dff port map(rst,clk,'1',Gz_aux(N/K),rGz_aux(N/K));
    regP_last: dff port map(rst,clk,'1',Pz_aux(N/K),rPz_aux(N/K));
